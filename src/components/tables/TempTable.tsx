    ...
    cell: info => {
      ...
      if (timestamp) {
        try {
          ...
          if (!isNaN(utcDate.getTime())) {
            ...
            timeDisplay = format(istDate, 'dd-MMM-yy HH:mm:ss');
          }
        } catch (error) {
      ...
      return (
        <div>
          ...
          <div className="text-[10px] text-[#808080]">{timeDisplay}</div>
        </div>
      );
    },
  }),
...
*/
  ...
  columnHelper.accessor(row => {
    const latitude = row.device_data?.lastValidLatitude;
    const longitude = row.device_data?.lastValidLongitude;
    return { latitude, longitude };
  }, {
    id: 'location',
    header: 'Location',
    cell: info => {
      const { latitude, longitude } = info.getValue();
      ...
      return (
        <button
          onClick={() => {
            const row = info.row.original;
            info.table.options.meta?.openMap(latitude, longitude, row.name);
          }}
...
columnHelper.accessor(row => {
  ...
  cell: info => {
    ...
    return `${Number(value).toLocaleString()} kms`;
  },
  size: 120,
}),
  columnHelper.accessor(row => {
  ...
  columnHelper.accessor(row => {
    ...
    header: 'Temperature',
    sortingFn: (rowA, rowB) => {
      const a = Number(rowA.getValue('temperature'));
      const b = Number(rowB.getValue('temperature'));
      if (isNaN(a)) return 1;
      if (isNaN(b)) return -1;
      return a - b;
    },
...
export function TempTable({ apiHash }: DeviceTableProps) {
  ...
  const table = useReactTable({
    ...
    columns,
    initialState: {
      sorting: [{ id: 'temperature', desc: false }]
    },
    state: {
      sorting,
      globalFilter,
    ...
    filterFns: {
      numericTemp: (row, columnId, filterValue) => {
        const temp = row.getValue(columnId);
        return temp !== '-' && !isNaN(Number(temp));
      }
    },
    ...
    meta: {
      openMap: (latitude: number, longitude: number, vehicleName: string) => {
        setMapModalState({
          isOpen: true,
          latitude,
          longitude,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter devices with valid temperature readings
  ...
  const exportToCSV = () => {
        ...
        .map(column => {
          ...
          switch (column.id) {
            case 'rowNumber':
              ...
              value = pageIndex * pageSize + rowIndex + 1;
              break;
            case 'timestamp':
              if (value) {
    ...
    a.href = url;
    a.download = `temperature-data-${format(new Date(), 'yyyy-MM-dd-HH-mm')}.csv`;
    a.click();
  ...
  return (
    ...
    <MapModal
      ...
      vehicleName={mapModalState.vehicleName}
    />
    </>
  );
}
